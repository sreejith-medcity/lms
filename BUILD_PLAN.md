...
| 3.11 | Announcements, feedback forms, testimonials | `/admin/announcements`, `/admin/feedback` | `Announcement`, `FeedbackForm`, `FeedbackResponse`, `Testimonial` | respective | Targeted at a batch or a course | `partial` — announcements target batches and appear on the learner dashboard. Feedback forms are built: a question builder, a star rating asked on the learner's own dashboard about a class they actually sat, and results with a response rate whose denominator is printed. Testimonials still missing |
