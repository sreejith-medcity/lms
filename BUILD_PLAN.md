...
| 1.12 | **Navigation tells the truth** | admin sidebar | — | — | Nothing navigates to a blank route | `done` — every entry now leads somewhere real |
...
| 2.5 | Coupons | `/admin/promo-codes`, checkout | `PromoCode`, `PromoCodeProduct`, `PromoRedemption` | `promocode.*` | Eligibility and redemption limits enforced atomically | `done` — percent with a ceiling or flat, per course or everything, date window, single-use or capped, per-person limit. The claim takes a row lock on the code before it counts, so two people paying in the same second queue rather than race; the redemption is reserved with the order and released if the payment fails. The discount comes off before GST |
...
| 2.12 | Abandoned carts | `/admin/carts` | `Cart`, `CartItem` | `sales.abandoned_cart` | Carts that go quiet are recoverable, with a nudge that is recorded | `done` — a cart is written when a learner opens the payment screen, aged after six hours of quiet, sorted by how many times they came back rather than by date, and nudged through the outbox. Converted automatically when they pay. No multi-item basket yet; that arrives with the WooCommerce absorption |
| 2.13 | Cheques | `/admin/cheques` | `Cheque`, `Payment` | `sales.cheques` | A cheque counts as collected only when it clears | `done` — recorded as a PENDING payment with the paper's details, cleared or bounced as separate recorded facts, duplicates refused, overdue ones flagged against the date on the cheque |
| 2.14 | Settlements | `/admin/settlements` | `Settlement`, `Payment` | `sales.settlements` | Collections reconciled against what the bank received | `done` — payout entered with gross, fee and GST; captured payments attached oldest first until the gross is used up, anything unmatched stays visibly unsettled. Razorpay's settlement API is a Phase 7 connection, and the shape here is the one it returns |
...
| 3.3 | Live classes and attendance | `/admin/sessions`, `/[id]`, `/admin/calendar` | `LiveSession`, `SessionRecurrence`, `Attendance` | `scheduling.sessions`, `scheduling.calendar`, `scheduling.mark_holiday` | Weekly series; attendance recorded on join with a ten-minute grace; a calendar a trainer can live in | `done` — plus a day, week, month and list calendar filtered by batch or trainer, drawn as a real time grid in the academy's own timezone, and holidays that call classes off with a flag so attendance stops counting them |
...
| 3.5 | Recordings | `/admin/recordings`, session page, `/learn/[p]` | `Recording`, `Asset` | `class_recording.*` | Published to the batch that sat the class | `done` — plus a library across every batch with search, batch and state filters, inline rename and bulk publish, so a term's recordings are reviewed in one pass |
...
| 3.11 | Announcements, feedback forms, testimonials | `/admin/announcements`, `/admin/feedback`, `/admin/testimonials`, `/admin/banners` | `Announcement`, `FeedbackForm`, `FeedbackResponse`, `Testimonial`, `Banner` | respective | Targeted at a batch or a course | `done` — announcements target batches; feedback forms have a question builder, a star rating asked on the learner's own dashboard about a class they actually sat, and results with a printed denominator; testimonials arrive unpublished from learners who finished a course and sit in an unread-first queue; banners are managed per placement with one shown at a time |
| 4.1 | Segments | `/admin/segments` | `Segment` | `marketing.segments` | A rule, recomputed, not a list somebody maintains | `done` — eight conditions joined with and or or, compiled to a query; the count carries the date it was worked out, and a campaign resolves its members at the moment it is prepared rather than trusting the cache |
| 4.2 | Community and discussions | `/admin/community`, `/learn/community` | `Community`, `CommunityPost`, `CommunityComment` | `community.*` | Posts, replies, moderation, and a room per course | `done` — learner text is escaped into paragraphs rather than filtered; a report is not a verdict and hides nothing on its own; course rooms are made on first use so there is no wall of empty ones |
| 4.3 | Points and referrals | `/admin/loyalty`, `/learn/wallet` | `WalletAccount`, `WalletTransaction`, `LoyaltyConfig`, `ReferralCode`, `Referral` | `settings.preferences` | A ledger, a ceiling, and credit that cannot become free courses | `done` — every change is a row with a reason; points come off the payable total inside the order's own transaction and are returned if the payment fails; redemption capped as a share of the order |
| 4.4 | Learner export | `/admin/learners/export` | — | `learner.learner_export` | One row per learner, and the export itself recorded | `done` — courses and batches joined into a cell so mail-merge works, collections and credit included, filterable by batch or course |
| 4.5 | Impersonation | learner page | `AuthSession`, `AuditLog` | `learner.learner_impersonate` | Support can see what the learner sees, visibly and briefly | `done` — an hour, learners only, a banner across every page, the staff session parked in a second cookie, both ends on the audit trail and the borrowed session destroyed on return |
| 4.6 | Instructor profiles | `/admin/instructors`, course page | `InstructorProfile` | `instructor.instructor_management` | The public page says who teaches and why that matters | `done` — headline, bio and subjects on the course page, plus an opt-out for trainers who would rather not be named |
| 4.7 | Learner portal menu | `/admin/settings/learner-portal` | `OrgSetting` | `settings.preferences` | Order and hide the learner's own navigation | `done` — stored as ordered keys, unknown keys dropped rather than rendered, and My learning cannot be removed |
...
| 6.1 | Preferences | `/admin/settings/preferences` | `OrgSetting` | `settings.preferences` | Settings that say what they do, and do it | `done` — declared in a registry rather than hand-laid, so search, defaults, per-setting history, reset and export all come from one place. Only values differing from the default are stored, so an improved default reaches every academy that never touched it. A setting nothing reads yet is labelled as such and refuses to save, rather than quietly doing nothing |
| 6.2 | Custom fields | `/admin/settings/custom-fields` | `CustomFieldDefinition`, `CustomFieldValue` | `settings.custom_fields` | Typed fields across eight entities, on signup and the office form | `done` — eight entities, seven types, before-or-after signup timing with the trade-off spelled out on the form, required flags per surface, ordering. The key is frozen at creation so renaming is free and answers are never orphaned; a field with answers can be switched off but not deleted |
| 6.3 | Notification matrix | `/admin/settings/notifications` | `NotificationSetting`, `MessageTemplate` | `settings.notifications` | Every event by every channel | `done` — sixteen events in four groups against four channels, saved a cell at a time. Events nothing emits yet are marked and say what they wait on, and a channel with no template behind it says so |
| 6.4 | Learner portal menu | `/admin/settings/learner-portal` | `OrgSetting` | `settings.preferences` | Order and hide the learner's navigation | `done` — see 4.7 |
| 6.5 | Grading | `/admin/settings/grading` | `GradeScale` | `settings.preferences` | A score becomes a grade the academy chose | `done` — bands with points and a meaning, checked as they are typed: a gap, an overlap or a backwards band cannot be saved, because a hole between 79 and 80 is invisible until a report card comes out blank in front of a parent. A live preview says what any score would be called |
| 6.6 | Website and policies | `/admin/settings/website` | `Organization.social`, `Policy` | `settings.organization` | Social links and the five policies a storefront needs | `done` — links render in the footer and a blank one simply does not appear; each policy says why it exists, shows its word count and last edit, and publishes to its existing public page |
| 6.7 | Brand | `/admin/settings` | `Organization` | `settings.organization` | The product wears the academy's brand | `done` — token layer rebuilt on the 2026 brand board: #322046 as the interface colour, #FDB85B as an accent reserved for the one action a page exists for, warm off-white canvas, purple-grey secondary ink, and a matching dark mode |
| 6.8 | Integrations | `/admin/settings/integrations` | `Integration` | `settings.integrations` | Credentials entered and stored safely, per academy | `done` — 49 providers across 11 categories, declared like the reports and settings so the next one is a few lines rather than a page. Secrets are sealed with AES-256-GCM under AUTH_SECRET before they are written and never read back, so a blank field means keep rather than clear; the environment still wins where it is set. Every card says one of three things: it runs, it will run when you fill it in, or the code for it is not written yet |
