...
| 1.12 | **Navigation tells the truth** | admin sidebar | — | — | Nothing navigates to a blank route | `done` — the full map is shown; unbuilt entries are inert, dimmed, dotted and explained by a legend, and become links as they are built |
...
| 2.6 | Instalments and fee tracking | `/admin/fees` | `Instalment`, `Payment` | `sales.fee_tracking` | Due dates, part payments, reminders | `partial` — plans, overdue totals, and a collected instalment written as a real payment. Reminders wait on messaging |
...
| 3.2 | Batches | `/admin/batches` | `Batch`, `BatchStaff`, `BatchModule` | `batches.*` | Roster, staff, per-batch curriculum | `partial` — create with branch, dates, capacity and one default per course, plus status changes inline. Staff assignment and per-batch curriculum still missing |
