...
export const INTEGRATIONS: IntegrationDef[] = [
  ...
  },
  {
    ...
    priority: 2,
    requires:
      'A Stripe account that can accept the currency you sell in (an Indian entity needs an Indian Stripe account). A restricted key with Checkout Sessions, and a webhook on checkout.session.completed pointing at /api/payments/stripe/webhook on this site.',
    purpose: 'International cards, for learners paying from outside India. Offered on the checkout page beside the Indian gateway when the setting under Payments says so.',
    status: 'wired',
    fallback: 'Learners abroad pay through the Indian gateway, which most foreign cards refuse.',
  ...
  },
  {
    ...
    priority: 2,
    requires:
      'A PayU merchant account: the merchant key and the salt for the environment you are on (test and production salts differ). Set surl/furl are not needed; this site sends its own.',
    purpose: 'An Indian gateway, common where a bank relationship already exists; EMI appears on its page when the merchant account has it. Chosen under Settings, Payments.',
    status: 'wired',
    fallback: 'Checkout uses Razorpay.',
    docsUrl: 'https://docs.payu.in/docs/payu-hosted-checkout',
    fields: [
      TEXT('merchantKey', 'Merchant key'),
      KEY('salt', 'Salt'),
      TEXT('environment', 'Environment', undefined, 'production or test.'),
    ],
  },
  {
    id: 'phonepe',
    ...
    priority: 3,
    requires:
      'A PhonePe business merchant ID with the salt key and salt index, and this site\'s /api/payments/phonepe/webhook as the callback. Onboarding is manual and takes a few days.',
    purpose: 'UPI-first checkout, which is what most walk-in learners reach for. Chosen under Settings, Payments.',
    status: 'wired',
    fallback: 'Checkout uses Razorpay.',
    docsUrl: 'https://developer.phonepe.com/v1/docs/pay-api',
    fields: [
      TEXT('merchantId', 'Merchant ID'),
      KEY('saltKey', 'Salt key'),
      TEXT('saltIndex', 'Salt index', undefined, 'Usually 1.'),
      TEXT('environment', 'Environment', undefined, 'production or sandbox.'),
    ],
  },
  {
    id: 'paypal',
  ...
  },
  {
    ...
    priority: 1,
    requires:
      'A reCAPTCHA v3 site registered for this domain (v3, not v2: a score rather than a puzzle). The site key and the secret key.',
    purpose: 'Keeps scripted sign-ups and enquiries out without asking real people to solve anything. Checked on the sign-up form and the enquiry form; a low score is refused and written to the integration history.',
    status: 'wired',
    fallback: 'The forms rely on rate limits and the honeypot field alone.',
  ...
  },
  {
    ...
    category: 'auth',
    priority: 2,
    requires: 'A Turnstile widget created in the Cloudflare dashboard for this domain, invisible mode. The site key and the secret key. Free.',
    purpose: 'The same job as reCAPTCHA without sending visitors to Google. If both cards are filled in, Turnstile is used.',
    status: 'wired',
    fallback: 'The forms rely on rate limits and the honeypot field alone.',
