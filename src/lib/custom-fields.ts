...
/** What a FILE field holds: the asset and enough about it to list without a lookup. */
...
export function fieldFile(value: unknown): FieldFile | null {
...
export const FIELD_FILE_MAX_BYTES = 15 * 1024 * 1024;
...
export async function storeFieldFile(input: {
  ...
  const { buildObjectKey, inferType, putObject, sanitiseFileName } = await import('@/lib/storage');
