...
export default async function HelpPage() {
  ...
  return (
    <div className="mx-auto max-w-4xl px-5 py-7">
      ...
      <p className="t-small faint mt-1">Ask the office anything about your fees, classes or account. It goes to your branch and you will see the reply here and by email.</p>

      <div className="mt-6 space-y-6">
        <Card>
          <h2 className="t-heading">Ask something</h2>
          <div className="mt-4">
        ...
        {open.length > 0 && (
          <section>
            <h2 className="t-heading mb-3">Open</h2>
            <Card padded={false}>
              <ul className="divide-y">
                {open.map((t) => (
                  <TicketRow key={t.id} t={t} timezone={tenant.timezone} />
                ))}
              </ul>
            </Card>
          </section>
        ...
        {done.length > 0 && (
          <section>
            <h2 className="t-heading mb-3">Earlier</h2>
            <Card padded={false}>
              <ul className="divide-y">
                {done.map((t) => (
                  <TicketRow key={t.id} t={t} timezone={tenant.timezone} />
...
function TicketRow({ t, timezone }: { t: { id: string; subject: string; category: string; status: string; lastMessageAt: Date; lastFromStaff: boolean; _count: { messages: number } }; timezone: string }) {
  return (
    <li className="flex flex-wrap items-center justify-between gap-3 px-5 py-3">
      <div className="min-w-0">
        <Link href={`/learn/help/${t.id}`} className="font-medium hover:underline">{t.subject}</Link>
        <p className="t-small faint">
          {categoryLabel(t.category)} · {t._count.messages} message{t._count.messages === 1 ? '' : 's'} · {t.lastFromStaff ? 'the office wrote' : 'you wrote'} {formatDateTime(t.lastMessageAt, timezone)}
        </p>
      </div>
