...
export async function GET(request: Request) {
  ...
  // Behind the host's proxy request.url is the process address; the
  // redirect must carry the academy's hostname or the phone lands nowhere.
  const origin = publicOrigin(request);
  const token = url.searchParams.get('token') ?? '';
  const claims = readHandoff(token);
  if (!tenant || !claims) return NextResponse.redirect(new URL('/login', origin));
  const now = Date.now();
  for (const [k, t] of used) if (t < now - 5 * 60_000) used.delete(k);
  if (used.has(claims.nonce)) return NextResponse.redirect(new URL('/login', origin));
  ...
  if (claims.sub.startsWith('parent:')) {
    ...
    if (!linked) return NextResponse.redirect(new URL('/parent/login', origin));
  ...
  if (outcome.status !== 'signed-in') return NextResponse.redirect(new URL('/login', origin));
  return NextResponse.redirect(new URL(claims.path, origin));
}
